source 'https://rubygems.org'

gem 'jekyll', '~> 3.7.3'
gem 'contentful-migrations'
gem 'rake'
gem 'bootstrap'
gem 'autoprefixer-rails'
gem 'font-awesome-sass',   '~> 5.0.1'
gem 'activesupport'

group :jekyll_plugins do
  # gem 'jekyll-contentful', '~> 1.4.0', git: 'https://github.com/crdschurch/jekyll-contentful.git', tag: '1.4.0'
  # gem 'jekyll-contentful', git: 'https://github.com/crdschurch/jekyll-contentful.git', branch: 'has-many-assets'
  gem 'jekyll-contentful', path: '/Users/wph/sites/ample/jekyll-contentful'
  gem 'jekyll-placeholders', '~> 0.0.1', git: 'https://github.com/ample/jekyll-placeholders.git'
  gem 'jekyll-assets'
end

group :development do
  gem 'pry'
end

